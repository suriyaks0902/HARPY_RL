...
class CassieEnv:
    ...
    def __init_reward_func(self):
        self.reward_names = [
            ...
            "r_hover_stability",
            "r_smooth_landing",
            "r_jump_height"  # New reward for jump height
        ]
        w_mpos = 15.0
        w_mvel_nonstand = 0.0
        w_mvel_stand = 15.0
        ...
        w_hover = 6.0
        w_landing = 8.0
        w_jump_height = 10.0  # High weight for jump height target
        ...
        self.reward_scales["thruster_use"] = -0.05  # Penalty for thruster usage
        self.reward_scales["hover_stability"] = -2.0  # Penalty for unstable hovering
        self.reward_scales["smooth_landing"] = -5.0  # Penalty for hard landing
        self.reward_scales["jump_height"] = -2.0  # Scale for height error
    ...
    def __reset_consts(self):
        ...
        self.init_height = self.qpos[2]  # Add this line to store initial height
    ...
    def __get_reward(self, acs):
        ...
        # Initialize all reward components to 1.0 (neutral value)
        r_thruster_use = 1.0
        ...
        metrics = {
            ...
            "thruster_left": thruster_action[0],
            "thruster_right": thruster_action[1],
            "jump_height": self.qpos[2] - self.init_height
        }
        ...
        #     # Add jump height reward when airborne
        #     if hasattr(self, 'config') and 'target_jump_height' in self.config:
        #         target_height = self.config['target_jump_height']
        #         current_height = self.qpos[2]
        #         achieved_height = current_height - self.init_height
        #         height_err = abs(achieved_height - target_height)
        #         r_jump_height = np.exp(-2.0 * height_err)
        #     # else:
        #     #     r_jump_height = 1.0
        # elif stage == "landing":
        ...
        #     # r_jump_height = 1.0
        if stage == "walking":
            r_thruster_use = np.exp(-0.1 * metrics["thruster_abs_sum"])
        elif stage == "airborne":
            r_thruster_use = np.exp(-0.01 * (metrics["thruster_mean"] - 0.5) ** 2)
            r_hover_stability = np.exp(-1.0 * np.sum(np.abs(self.qvel[[3, 4]])))
            if hasattr(self, 'config') and 'target_jump_height' in self.config:
                height_err = abs(metrics["jump_height"] - self.config['target_jump_height'])
                r_jump_height = np.exp(-2.0 * height_err)
        elif stage == "landing":
            r_thruster_use = np.exp(-0.05 * metrics["thruster_abs_sum"])
            r_smooth_landing = np.exp(-np.abs(self.qvel[2]))

        # Combine with existing rewards array
        rewards = np.array(
            [
                r_mpos,
                r_mvel,
        ...
        # # Add thruster stats to info dict
        # reward_dict["thruster_sum"] = np.sum(thruster_action)
        # reward_dict["thruster_mean"] = np.mean(thruster_action)
        # reward_dict["thruster_max"] = np.max(thruster_action)
        # reward_dict["thruster_min"] = np.min(thruster_action)
        # reward_dict["thruster_abs_sum"] = np.sum(np.abs(thruster_action))
        # reward_dict["thruster_left"] = thruster_action[0]
        # reward_dict["thruster_right"] = thruster_action[1]

        # Add metrics to reward dict
        reward_dict.update(metrics)
        
        if stage == "airborne":
            current_height = self.qpos[2]
            achieved_height = current_height - self.init_height
            reward_dict["jump_height"] = achieved_height
