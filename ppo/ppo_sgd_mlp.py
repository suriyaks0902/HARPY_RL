...
from optimization.policy_optimizer import OptimizedPolicy
import os
...
def learn(
    ...
    *,
    ...
    # Add new reward buffers
    rewbuffer_thruster = deque(maxlen=100)
    rewbuffer_hover = deque(maxlen=100)
    rewbuffer_landing = deque(maxlen=100)
    rewbuffer_jump_height = deque(maxlen=100)
    ...
    while True and max_iters != 1:
        ...
        lrlocal = (
            ...
            seg["ep_rets_detail"][:, 13],  # thruster
            seg["ep_rets_detail"][:, 14],  # hover
            seg["ep_rets_detail"][:, 15],  # landing
            seg["ep_rets_detail"][:, 16],  # jump height
        )  # local values
        ...
        rewbuffer_thruster.extend(rews_thruster)
        rewbuffer_hover.extend(rews_hover)
        rewbuffer_landing.extend(rews_landing)
        rewbuffer_jump_height.extend(rews_jump_height)
        ...
        logger.record_tabular("rewards/EpRewThrusterUse", np.mean(rewbuffer_thruster))
        logger.record_tabular("rewards/EpRewHoverStability", np.mean(rewbuffer_hover))
        logger.record_tabular("rewards/EpRewSmoothLanding", np.mean(rewbuffer_landing))
        logger.record_tabular("rewards/EpRewJumpHeight", np.mean(rewbuffer_jump_height))
        ...
        if MPI.COMM_WORLD.Get_rank() == 0:
            # logger.dump_tabular()
            print("Optimizing policy with TensorRT...")
            # Generate save path based on iteration
            save_path = os.path.join(ROOT_PATH, 'ckpts', 
                                f'model_{timesteps_so_far}')
            try:
                optimized_pi = OptimizedPolicy(pi, save_path)
                print("TensorRT optimization successful!")
                return optimized_pi
            except Exception as e:
                print(f"TensorRT optimization failed: {e}")
                print("Falling back to original policy")
                return pi
